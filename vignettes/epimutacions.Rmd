...
```{r ex08, message = FALSE, eval = FALSE}
...
```{r annot, eval = TRUE}
# annotate_epimutations() queries Ensembl/biomaRt over the network. If the
# service is momentarily unavailable, fall back gracefully so the vignette
# still builds; the annotation tables below are shown only when it succeeds.
rst_mvo <- tryCatch(
    annotate_epimutations(epi_mvo, omim = TRUE),
    error = function(e) {
        message("Skipping online annotation (Ensembl not reachable): ",
                conditionMessage(e))
        NULL
    }
)
ann_ok <- !is.null(rst_mvo)
```
