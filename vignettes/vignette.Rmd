...
A large part of the development of this package was produced along the [European BioHackathon 2020](https://www.biohackathon-europe.org/), where we worked on implementing it  to enable analyzing epimutations to the rare disease community. We worked on three main areas: (1) implementation of algorithms to detect epimutations; (2) implementation of reporting functions; (3) testing and validation of the package.
...
The `epi_parameters()` function  is  useful to set the parameters for each method mentioned above. 
The following table describes the arguments: 

| Method | Parameter | Description |
|:------:|:------------------|:------------------|
| `"manova"`      | `pvalue_cutoff` | the threshold p value to select which CpG regions are  outliers |
| `"mlm"`         |  `pvalue_cutoff` | the threshold p value to select which CpG regions are  outliers |
| `iso.forest`    | `outlier_score_cutoff` | the threshold outlier score to select which CpG regions are outliers | 
| `"mahdist.mcd"` | `nsamp` | the number of subsets used for initial estimates |
|                 | `window_sz`   | the maximum distance between CpGs to be considered in the same DMR |
|   `"barbosa"`   | `offset_mean`, `offset_abs` | the upper and lower threshold to consider a CpG an outlier | 
...
We called `"qn` to the method described in [@rousseeuw1993alternatives]. It computes the robust scale estimator.
...
The following three columns provide some statistic from the detection process:
 ...
 1. `outlier_score`:
   ...
   * For methods `barbosa` and `mahdistmcd` is filled with `NA`.
...
A visualization method is in order to locate the epimutations along the genome. This can be archived by using the function `epi_plot`, already included in `epimutacions`.

```{r, plot_mvo, fig.width=7, message=FALSE}
epi_plot(as.data.frame(epi_mvo[1,]), methy)
```
