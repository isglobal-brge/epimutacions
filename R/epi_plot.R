...
##From and to arguments
  ...
  ###
  beta_values$status <- ifelse(beta_values$variable == dmr$sample, "case", "control")
  beta_values$color <- ifelse(beta_values$status == "case","red","black")
...
#Plot epimutations
  ...
  plot <- plot_cpg_names + 
          ggplot2::ylim(0,1) +  
          ggplot2::scale_colour_manual(name="Status",values=c("red","black","darkblue"))
