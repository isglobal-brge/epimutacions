...
#' 
#' @export
plot_epi <- function(methy, epi_res, sam, chr, genome = "hg19", gene_annotation = FALSE, from = NULL, to = NULL) {
	if(length(chr) != 1) {
	...
	if(!is.null(genes)){
		...
		gene_track <- Gviz::GeneRegionTrack(genes, chromosome = chr, start = min(start(gr)), end = max(end(gr)), name = "Genes", transcriptAnnotation = "symbol", background.title = "#7EA577")				
		highlight_track <- Gviz::HighlightTrack(trackList = list(genome_track, gene_track) , start = min(start(gr)), width = max(end(gr)) - min(start(gr)),  chromosome = chr)
		
	}
		if(is.null(from) & is.null(to)){
			Gviz::plotTracks(list(highlight_track), from = min(start(gr)) - 500000, to = max(end(gr))+500000)
		}else{
			Gviz::plotTracks(list(ideo_track, genome_track, gene_track, highlight_track), from = from, to = to+2)	
		}
		}
