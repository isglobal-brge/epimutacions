...
#'
#' @importFrom minfi combineArrays
...
PCA_correction <- function(cases, controls, prange = 40000) {
    ...
    p <- intersect( Biobase::featureNames(cases), Biobase::featureNames(controls))
    ## found 51 overlapping featureNames then combine the sets
    all_samples <- minfi::combineArrays( cases[p, ], controls[p, ])
    ...
    m <- minfi::getM(all_samples)
    res <- residuals(limma::lmFit(m, pcs[, seq_len(ndim)]), m)
    beta <- minfi::ilogit2(res)
    SummarizedExperiment::assay(all_samples) <- beta
